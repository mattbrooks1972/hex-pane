...
export default class HexPaneView {
	constructor(serializedState) {
		...
		this.invisibles = {
			eol: atom.config.get('hex-pane.invisibles.eol'),
			cr: atom.config.get('hex-pane.invisibles.cr'),
			tab: atom.config.get('hex-pane.invisibles.tab'),
			space: atom.config.get('hex-pane.invisibles.space')
		};
	...
	toFormat(string) {
		...
		for(i = 0; i < string.length; i++) {
			switch(string[i]) {
				case ' ':
					buffer += this.invisibles.space;
					break;
				case '\t':
					buffer += this.invisibles.tab;
					break;
				case '\r':
					buffer += this.invisibles.cr;
					break;
				case '\n':
					buffer += this.invisibles.eol;
					break;
				default:
					buffer += string[i];
					break;
